...
module.exports = {
	...
	async execute(interaction) {
		...
		const { channelId, highScore, highestAlbum, roundsCompleted } = await getServerStats(interaction.guildId);
		if (!channelId) {
			await interaction.reply("No channel has been set for this server yet.");
			return;
		}
		const channel = interaction.guild.channels.cache.get(channelId);
		await interaction.reply(`High score: ${highScore}\nListening channel: ${channel}\nHighest album: ${highestAlbum || "(not started)"}\nRounds completed: ${roundsCompleted || 0}`);
